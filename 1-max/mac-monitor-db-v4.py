...
# 알림: macOS에서는 osascript 사용, Windows에서는 plyer 사용
try:
...
# ImageGrab: macOS에서도 동작하지만 import 방식 분기
try:
...
def open_file_or_folder(path):
    ...
    try:
        ...
        logging.error(f"파일/폴더 열기 실패: {path}, 오류: {e}")
        webbrowser.open("file://" + path)

def send_notification(title, message, app_name="실거래가 모니터", timeout=10):
    """크로스 플랫폼 시스템 알림"""
    try:
        if IS_MAC:
            # macOS: osascript로 알림 표시
            script = f'display notification "{message}" with title "{title}"'
            subprocess.run(['osascript', '-e', script], capture_output=True)
        elif notification:
            notification.notify(title=title, message=message, app_name=app_name, timeout=timeout)
        else:
            logging.info(f"알림: {title} - {message}")
    except Exception as e:
...
def get_default_download_path():
    """OS별 기본 다운로드 경로"""
    if IS_MAC:
        return os.path.expanduser("~/Downloads/real_estate_monitor")
    else:
        return "C:\\Download"

def get_default_lawdong_path():
    """OS별 기본 법정동 파일 경로"""
    if IS_MAC:
        return os.path.expanduser("~/law-dong/law-dong.txt")
    else:
        return "C:/law-dong/law-dong.txt"

def get_mac_font():
    """macOS 호환 한글 폰트 반환"""
    if IS_MAC:
        # macOS에서 사용 가능한 한글 폰트 목록
        mac_fonts = ['Apple SD Gothic Neo', 'AppleGothic', 'NanumGothic', 'Helvetica']
        return mac_fonts[0]
    return get_mac_font()
# (선택) 월 이동 정확도 개선 시 사용
...
def init_database(db_path):
    ...
    # 날짜별 신고가 이력 테이블
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS high_price_log (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            new_floor TEXT,
            new_dong TEXT,
            region_84_rank INTEGER,
            region_59_rank INTEGER,
            region_name TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            UNIQUE(detected_date, list_name, apt_name)
        )
    """)
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_hpl_date ON high_price_log(detected_date)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_hpl_list ON high_price_log(list_name)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_hpl_sigungu ON high_price_log(sigungu)")
...
class RealEstateMonitorApp:
    def __init__(self):
        ...
        self.lawdong_path = get_default_lawdong_path()
        ...
        self.html_download_path = "/Users/leehyungjun/python work"  # HTML 저장 경로
        ...
        # 핫플레이스 정보 기본값 (load_settings에서 덮어씀)
        ...
        # ★ 전체 통합 시 포함할 리스트 필터 (None = 전체 포함)
        self.unified_filter_lists = None  # None이면 모든 리스트 포함, list이면 해당 리스트만
        ...
        # 마지막 검색 지역 저장 변수 (load_settings에서 덮어씀)
    ...
    UNIFIED_LIST_NAME = "★ 전체 통합"  # 가상 통합 리스트 이름

    @property
    def monitored_apts(self):
        # 전체 통합 가상 리스트: 선택된 리스트의 단지를 합산하여 반환
        if self.active_list.get() == self.UNIFIED_LIST_NAME:
            combined = []
            for name, apts in self.monitored_lists["lists"].items():
                if name == self.UNIFIED_LIST_NAME:
                    continue
                # unified_filter_lists가 설정된 경우 해당 리스트만 포함
                if self.unified_filter_lists is not None and name not in self.unified_filter_lists:
                    continue
                combined.extend(apts)
            return combined
        return self.monitored_lists["lists"].setdefault(self.active_list.get(), [])

    @monitored_apts.setter
    def monitored_apts(self, new_list):
        # 전체 통합 가상 리스트는 직접 수정 불가
        if self.active_list.get() == self.UNIFIED_LIST_NAME:
            return
    ...
    def load_notifications_history(self):
        ...
        print(f"[신고가 로드] 경로 확인: {self.notifications_history_file}")
        print(f"[신고가 로드] 파일 존재: {os.path.exists(self.notifications_history_file)}")
    ...
    def analyze_kb_excel(self):
        """KB 시세 엑셀 '1.매매증감' 시트에서 지역별 상승률 분석
        Returns:
            dict or None: {
                'weekly_top3': [(지역명, 값), ...],   # 최근 주간 TOP3
                'month3_top3': [(지역명, 합계), ...],  # 3개월 누적 TOP3
                'month6_top3': [(지역명, 합계), ...],  # 6개월 누적 TOP3
                'year1_top3': [(지역명, 합계), ...],   # 1년 누적 TOP3
                'latest_date': str                      # 최신 데이터 날짜
            }
        """
        if not self.kb_excel_path or not os.path.exists(self.kb_excel_path):
            logging.warning(f"KB 엑셀 파일 없음: {self.kb_excel_path}")
            return None
        ...
        try:
            ...
            # 엑셀 파일이 열려있을 수 있으므로 임시 파일로 복사 후 읽기
            temp_dir = tempfile.mkdtemp()
            temp_file = os.path.join(temp_dir, 'kb_temp.xlsx')
            try:
                # 크로스 플랫폼: shutil.copy2로 파일 복사
                shutil.copy2(self.kb_excel_path, temp_file)

            except Exception as e:
                logging.error(f"KB 엑셀 파일 복사 실패: {str(e)}")
                try:
            ...
            for name in wb.sheetnames:
                if '매매증감' in name:
                    ...
                    break
            if not sheet_name:
                logging.error(f"KB 엑셀에서 '매매증감' 시트를 찾을 수 없음. 시트 목록: {wb.sheetnames}")
                wb.close()
                try:
                    os.remove(temp_file)
                    os.rmdir(temp_dir)
            ...
            if len(all_rows) < 3:
                logging.error("KB 엑셀 데이터 행이 부족합니다.")
                wb.close()
                try:
                    os.remove(temp_file)
                    os.rmdir(temp_dir)
            ...
            header_row = all_rows[1]  # 2행 (0-indexed: 1)
            # 먼저 원본 헤더를 읽은 후, 시/도 접두어를 붙여줌
            raw_headers = []
            for cell in header_row[1:]:  # B열부터
                val = cell.value
                if val is not None:
                    raw_headers.append(str(val).strip())
                else:
                    raw_headers.append("")

            # 좌측으로 이동하며 가장 가까운 '시'/'도' 찾아서 접두어 붙이기
            last_parent = ""  # 가장 최근의 시/도 이름
            for i, name in enumerate(raw_headers):
                if not name:
                    headers.append("")
                    continue
                # 해당 셀 자체가 '시' 또는 '도'로 끝나면 → 상위 지역으로 기억
                # (예: 서울시, 경기도 → O / 도봉구, 시흥시 내부 구 → X)
                if name.endswith('시') or name.endswith('도'):
                    last_parent = name
                    headers.append(name)
                else:
                    # 구/군 등 하위 지역 → 직전 시/도를 접두어로 붙임
                    if last_parent:
                        headers.append(f"{last_parent} {name}")
                    else:
                        headers.append(name)

            # 3행부터 데이터 읽기: (날짜, [값들])
            data_rows = []
            for row in all_rows[2:]:  # 3행부터
            ...
            if not data_rows:
                logging.error("KB 엑셀에서 유효한 데이터 행을 찾을 수 없음")
                return None

            latest_date = data_rows[-1][0]
            latest_values = data_rows[-1][1]

            # 제외할 열 (상위 지역 → 하위 구/군으로 세분화되므로 중복 방지)
            # CJ, CK, CP, CT, CX, DA, DH, DN, ED 열 (B열=index 0 기준)
            exclude_cols = {86, 87, 92, 96, 100, 103, 110, 116, 132}

            def get_top3(values_list):
                """헤더와 값 리스트에서 상위 3개 반환 (제외 열 제외)"""
                paired = []
                for i, val in enumerate(values_list):
                    if i in exclude_cols:
                        continue
                    if i < len(headers) and headers[i]:
                        paired.append((headers[i], val))
                paired.sort(key=lambda x: x[1], reverse=True)
                return paired[:3]

            def get_sum_top3(n_rows):
                """최근 n_rows 행의 합계로 상위 3개 반환"""
                target_rows = data_rows[-n_rows:] if len(data_rows) >= n_rows else data_rows
                col_count = len(headers)
                col_sums = [0.0] * col_count
                for _, vals in target_rows:
                    for i in range(min(len(vals), col_count)):
                        col_sums[i] += vals[i]
                return get_top3(col_sums)

            result = {
                'weekly_top3': get_top3(latest_values),
                'month3_top3': get_sum_top3(13),
                'month6_top3': get_sum_top3(26),
                'year1_top3': get_sum_top3(52),
            ...
            logging.error("openpyxl 패키지가 설치되어 있지 않습니다. pip install openpyxl")
            messagebox.showerror("오류", "openpyxl 패키지가 필요합니다.\npip install openpyxl")
            return None
        except Exception as e:
            logging.error(f"KB 엑셀 분석 중 오류: {str(e)}")
            import traceback
            logging.error(traceback.format_exc())
            return None
    ...
    def setup_fonts(self):
        """폰트 설정 (2pt씩 확대) - macOS/Windows 호환"""
        font_name = get_mac_font()
        self.font_normal = (font_name, 11)
        self.font_large  = (font_name, 13)
        self.font_title  = (font_name, 16, 'bold')
        self.font_button = (font_name, 11)
    ...
    def save_monitored_apts(self):
        ...
        try:
            ...
            # DB에는 있지만 메모리에서 삭제된 리스트 제거
            current_names = list(self.monitored_lists["lists"].keys())
            if current_names:
                placeholders = ",".join("?" * len(current_names))
                cursor.execute(f"SELECT id, name FROM monitoring_lists WHERE name NOT IN ({placeholders})", current_names)
                removed_lists = cursor.fetchall()
                for removed_id, removed_name in removed_lists:
                    cursor.execute("DELETE FROM monitoring_lists WHERE id = ?", (removed_id,))
                    logging.info(f"DB에서 삭제된 리스트 제거: {removed_name}")
    ...
    def open_backup_folder(self):
        ...
        try:
            os.makedirs(self.backup_dir, exist_ok=True)
            open_file_or_folder(self.backup_dir)
        except Exception as e:
    ...
    def setup_gui(self):
        ...
        ttk.Button(button_frame, text="📅 신고가 히스토리 HTML", style='Accent.TButton',
                  command=self.show_high_price_history_dialog).pack(side="right", padx=(0, 5))
        ...
        # 콤보박스 (리스트 선택)
        _real_names = sorted(k for k in self.monitored_lists["lists"].keys()
                             if k != self.UNIFIED_LIST_NAME)
        self.list_combobox = ttk.Combobox(
            bar_left,
            values=[self.UNIFIED_LIST_NAME] + _real_names,
        ...
        # ★ 전체 통합 시 포함 리스트 선택 버튼 (통합 선택 시에만 표시)
        self.btn_unified_filter = ttk.Button(
            bar_left, text="☑ 포함 리스트 선택", style='Accent.TButton',
            command=self.show_unified_filter_dialog
        )
        self.btn_unified_filter.pack(side="left", padx=(8, 0))
        # 초기 상태: 통합 리스트 선택 여부에 따라 표시/숨김
        if self.active_list.get() != self.UNIFIED_LIST_NAME:
            self.btn_unified_filter.pack_forget()
    ...
    def refresh_list_combobox_values(self):
        # 실제 리스트 이름 (통합 제외)
        real_names = sorted(
            k for k in self.monitored_lists["lists"].keys()
            if k != self.UNIFIED_LIST_NAME
        )
        # 통합 리스트를 항상 맨 앞에 배치
        all_values = [self.UNIFIED_LIST_NAME] + real_names
        self.list_combobox['values'] = all_values
        # active_list 값이 사라진 경우 보정
        if self.active_list.get() not in all_values:
            if real_names:
                self.active_list.set(real_names[0])
            else:
    ...
    def edit_apt_data(self, event):
        ...
        ttk.Label(frame, text="(형식: YYYY-MM-DD)", font=(get_mac_font(), 8)).grid(
    ...
    def on_active_list_changed(self, event=None):
        # 리스트 변경 시 트리 갱신 + 저장
        # 전체 통합 가상 리스트는 DB에 저장하지 않음
        if self.active_list.get() != self.UNIFIED_LIST_NAME:
            self.save_monitored_apts()
        self.save_last_active_list()
        # ★ 전체 통합 선택 시 포함 리스트 선택 버튼 표시/숨김
        if self.active_list.get() == self.UNIFIED_LIST_NAME:
            self.btn_unified_filter.pack(side="left", padx=(8, 0))
        else:
            self.btn_unified_filter.pack_forget()
            self.unified_filter_lists = None  # 다른 리스트로 전환 시 필터 초기화
        self.update_apt_tree()
        # '부동산 최신 근황' 버튼: '서울 수도권' 또는 전체 통합 리스트에서 활성화
        if self.active_list.get() in ("서울 수도권", self.UNIFIED_LIST_NAME):
            self.btn_fear_greed.state(['!disabled'])
        else:
            self.btn_fear_greed.state(['disabled'])
        if self.active_list.get() == self.UNIFIED_LIST_NAME:
            filter_info = f" ({len(self.unified_filter_lists)}개 리스트 선택)" if self.unified_filter_lists is not None else " - 모든 지역 단지 합산 표시"
            self.status_var.set(f"★ 전체 통합 리스트 (읽기 전용){filter_info}")
        else:
            self.status_var.set(f"리스트 전환: {self.active_list.get()}")
    
    def show_unified_filter_dialog(self):
        """★ 전체 통합 시 포함할 리스트를 체크박스로 선택하는 팝업 다이얼로그"""
        real_names = sorted(
            k for k in self.monitored_lists["lists"].keys()
            if k != self.UNIFIED_LIST_NAME
        )
        if not real_names:
            messagebox.showinfo("알림", "선택 가능한 리스트가 없습니다.", parent=self.root)
            return

        dialog = tk.Toplevel(self.root)
        dialog.title("포함할 리스트 선택")
        dialog.resizable(False, False)
        dialog.grab_set()

        ttk.Label(dialog, text="★ 전체 통합에 포함할 리스트를 선택하세요:",
                  font=("", 10, "bold")).pack(padx=16, pady=(14, 6), anchor="w")

        # 체크박스 변수 생성 (현재 필터 상태 반영)
        check_vars = {}
        frame_checks = ttk.Frame(dialog)
        frame_checks.pack(padx=16, pady=4, fill="x")

        for name in real_names:
            var = tk.BooleanVar()
            # unified_filter_lists가 None이면 전체 선택, 아니면 포함 여부
            if self.unified_filter_lists is None or name in self.unified_filter_lists:
                var.set(True)
            else:
                var.set(False)
            check_vars[name] = var
            cb = ttk.Checkbutton(frame_checks, text=name, variable=var)
            cb.pack(anchor="w", pady=2)

        # 전체선택 / 전체해제 버튼
        btn_frame_top = ttk.Frame(dialog)
        btn_frame_top.pack(padx=16, pady=(6, 2), fill="x")
        ...
        ttk.Button(btn_frame_top, text="전체 선택", command=select_all).pack(side="left", padx=(0, 6))
        ttk.Button(btn_frame_top, text="전체 해제", command=deselect_all).pack(side="left")

        ttk.Separator(dialog, orient="horizontal").pack(fill="x", padx=8, pady=8)

        def on_confirm():
            selected = [name for name, var in check_vars.items() if var.get()]
            if not selected:
                messagebox.showwarning("경고", "최소 1개 이상의 리스트를 선택해야 합니다.", parent=dialog)
                return
            # 전체 선택이면 필터 해제(None), 일부만 선택이면 필터 적용
            if len(selected) == len(real_names):
                self.unified_filter_lists = None
                self.status_var.set("★ 전체 통합 리스트 (읽기 전용) - 모든 지역 단지 합산 표시")
            else:
                self.unified_filter_lists = selected
                self.status_var.set(f"★ 전체 통합 리스트 (읽기 전용) ({len(selected)}개 리스트 선택)")
            self.update_apt_tree()
            dialog.destroy()

        btn_frame_bot = ttk.Frame(dialog)
        btn_frame_bot.pack(padx=16, pady=(0, 14))
        ttk.Button(btn_frame_bot, text="확인", style='Accent.TButton', command=on_confirm).pack(side="left", padx=(0, 8))
        ttk.Button(btn_frame_bot, text="취소", command=dialog.destroy).pack(side="left")
    ...
    def rename_current_list(self):
        ...
        if cur == self.UNIFIED_LIST_NAME:
            messagebox.showinfo("알림", "전체 통합 리스트는 이름 변경할 수 없습니다.")
            return
    ...
    def delete_current_list(self):
        cur = self.active_list.get()
        if cur == self.UNIFIED_LIST_NAME:
            messagebox.showinfo("알림", "전체 통합 리스트는 삭제할 수 없습니다.")
            return
    ...
    def move_selected_to_list(self):
        if self.active_list.get() == self.UNIFIED_LIST_NAME:
            messagebox.showinfo("알림", "전체 통합 리스트에서는 아파트를 직접 이동할 수 없습니다.\n개별 리스트를 선택해주세요.")
            return
        selection = self.apt_tree.selection()
        if not selection:
            ...
            return
        cur = self.active_list.get()
        names = [n for n in sorted(self.monitored_lists["lists"].keys()) if n != cur and n != self.UNIFIED_LIST_NAME]
        if not names:
    ...
    def save_previous_high_to_db(self, apt_list):
        ...
        try:
            ...
            # 메모리 히스토리에 추가 (list_name 저장: 히스토리 재생 시 파일명 결정에 사용)
            saved_list_name = self.active_list.get() if hasattr(self, 'active_list') else ''
            notification_data = {'timestamp': current_time, 'apt_list': apt_list.copy(), 'list_name': saved_list_name}
    ...
    def show_high_price_history_dialog(self):
        """날짜/지역 선택 후 해당 날짜의 신고가 리스트 HTML을 재생성하는 다이얼로그"""
        # 광역 묶음 정의: 표시명 → sido 목록
        REGION_GROUPS = {
            '수도권(서울+경기+인천)': ['서울특별시', '경기도', '인천광역시'],
            '서울특별시': ['서울특별시'],
            '경기도': ['경기도'],
            '인천광역시': ['인천광역시'],
            '부산광역시': ['부산광역시'],
        ...
        try:
            ...
            if not dates:
                messagebox.showinfo("신고가 히스토리", "저장된 신고가 이력이 없습니다.\n모니터링 갱신 시 자동으로 저장됩니다.")
                return
            ...
            # ── 날짜 ──
            ttk.Label(dialog, text="날짜 선택").grid(row=0, column=0, sticky='w', **pad)
            date_var = tk.StringVar(value=dates[0])
            date_cb = ttk.Combobox(dialog, textvariable=date_var, values=dates, state='readonly', width=24)
            date_cb.grid(row=0, column=1, sticky='w', **pad)

            # ── 광역 필터 (sido / 묶음) ──
            ttk.Label(dialog, text="광역 필터").grid(row=1, column=0, sticky='w', **pad)

            def _load_sido_options(*_):
                sel_date = date_var.get()
                cursor.execute("""
                    SELECT DISTINCT sido FROM high_price_log
                    WHERE detected_date = ? AND sido IS NOT NULL AND sido != ''
                    ORDER BY sido
                """, (sel_date,))
                sidos_in_db = {r[0] for r in cursor.fetchall()}

                options = ['전체']
                # 수도권 묶음이 데이터에 존재하면 최상위에
                sudo_sidos = set(REGION_GROUPS['수도권(서울+경기+인천)'])
                if sidos_in_db & sudo_sidos:
                    options.append('수도권(서울+경기+인천)')
                # 나머지 개별 sido
                for label, sidos in REGION_GROUPS.items():
                    if label == '수도권(서울+경기+인천)':
                        continue
                    if sidos_in_db & set(sidos):
                        options.append(label)

                sido_cb['values'] = options
                sido_var.set('전체')
                _load_sigungu_options()

            sido_var = tk.StringVar(value='전체')
            sido_cb = ttk.Combobox(dialog, textvariable=sido_var, state='readonly', width=24)
            sido_cb.grid(row=1, column=1, sticky='w', **pad)

            # ── 시군구 필터 (선택) ──
            ttk.Label(dialog, text="시군구 필터 (선택)").grid(row=2, column=0, sticky='w', **pad)

            def _load_sigungu_options(*_):
                sel_date = date_var.get()
                sel_sido_label = sido_var.get()

                if sel_sido_label == '전체':
                    sido_filter = []
                else:
                    sido_filter = REGION_GROUPS.get(sel_sido_label, [sel_sido_label])

                if sido_filter:
                    placeholders = ','.join('?' * len(sido_filter))
                    cursor.execute(f"""
                        SELECT DISTINCT sigungu FROM high_price_log
                        WHERE detected_date = ? AND sido IN ({placeholders})
                          AND sigungu IS NOT NULL AND sigungu != ''
                        ORDER BY sigungu
                    """, [sel_date] + sido_filter)
                else:
                    cursor.execute("""
                        SELECT DISTINCT sigungu FROM high_price_log
                        WHERE detected_date = ? AND sigungu IS NOT NULL AND sigungu != ''
                        ORDER BY sigungu
                    """, (sel_date,))

                sigungu_cb['values'] = ['전체'] + [r[0] for r in cursor.fetchall()]
                sigungu_var.set('전체')

            sigungu_var = tk.StringVar(value='전체')
            sigungu_cb = ttk.Combobox(dialog, textvariable=sigungu_var, state='readonly', width=24)
            sigungu_cb.grid(row=2, column=1, sticky='w', **pad)
            sido_cb.bind('<<ComboboxSelected>>', _load_sigungu_options)

            # ── 리스트 필터 ──
            ttk.Label(dialog, text="리스트 필터 (선택)").grid(row=3, column=0, sticky='w', **pad)

            def _load_lists(*_):
                sel_date = date_var.get()
                cursor.execute("""
                    SELECT DISTINCT list_name FROM high_price_log
                    WHERE detected_date = ? ORDER BY list_name
                """, (sel_date,))
                list_cb['values'] = ['전체'] + [r[0] for r in cursor.fetchall()]
                list_var.set('전체')

            list_var = tk.StringVar(value='전체')
            list_cb = ttk.Combobox(dialog, textvariable=list_var, state='readonly', width=24)
            list_cb.grid(row=3, column=1, sticky='w', **pad)

            def _on_date_change(e):
                _load_sido_options()
                _load_lists()

            date_cb.bind('<<ComboboxSelected>>', _on_date_change)
            _load_sido_options()
            _load_lists()
            ...
            def _on_download():
                ...
                sql = "SELECT * FROM high_price_log WHERE detected_date = ?"
                params = [sel_date]
                if sido_filter:
                    placeholders = ','.join('?' * len(sido_filter))
                    sql += f" AND sido IN ({placeholders})"
                    params.extend(sido_filter)
                if sel_sigungu != '전체':
                    sql += " AND sigungu = ?"
                    params.append(sel_sigungu)
                if sel_list != '전체':
                    sql += " AND list_name = ?"
                    params.append(sel_list)
                ...
                def _row_to_apt(r):
                    return {
                        ...
                        'location_dong': r['dong'] or '',
                        'build_year': r['build_year'] or '',
                        'old_price': r['old_price'] or 0,
                        'old_date': r['old_date'] or '',
                        'old_floor': r['old_floor'] or '',
                        'old_dong': r['old_dong'] or '',
                        'new_price': r['new_price'],
                        'date': r['new_date'] or '',
                        'floor': r['new_floor'] or '',
                        'dong': r['new_dong'] or '',
                        'region_84_rank': r['region_84_rank'],
            ...
            logging.error(f"신고가 히스토리 다이얼로그 오류: {e}")
        ...
        def on_double_click(event):
            ...
            if selection:
                ...
                if 0 <= idx < len(sorted_history):
                    ...
                    stored_list_name = selected_notification.get('list_name', '')
                    if apt_list:
    ...
    def show_settings_dialog(self):
        ...
        # HTML 저장 경로
        ttk.Label(settings, text="HTML 저장 경로:", background=self.palette['surface'], foreground=self.palette['text_primary']).grid(row=5, column=0, sticky="w", padx=10, pady=10)
        html_path_var = tk.StringVar(value=self.html_download_path)
        ttk.Entry(settings, textvariable=html_path_var, width=40).grid(row=5, column=1, padx=5, pady=10)
        def select_html_path():
            path = filedialog.askdirectory(initialdir=self.html_download_path)
            if path:
                html_path_var.set(path)
        ttk.Button(settings, text="찾아보기", style='Accent.TButton', command=select_html_path).grid(row=5, column=2, padx=5, pady=10)

        # KB 시세 엑셀 파일 경로
        ttk.Label(settings, text="KB 시세 엑셀 파일:", background=self.palette['surface'], foreground=self.palette['text_primary']).grid(row=6, column=0, sticky="w", padx=10, pady=10)
        kb_excel_var = tk.StringVar(value=self.kb_excel_path)
        ttk.Entry(settings, textvariable=kb_excel_var, width=40).grid(row=6, column=1, padx=5, pady=10)
        def select_kb_excel():
            init_dir = os.path.dirname(self.kb_excel_path) if self.kb_excel_path else os.getcwd()
            path = filedialog.askopenfilename(
                initialdir=init_dir,
                title="KB 시세 엑셀 파일 선택",
        ...
        def save_settings():
            ...
            # HTML 저장 경로
            new_html_path = html_path_var.get()
            if new_html_path:
                os.makedirs(new_html_path, exist_ok=True)
                self.html_download_path = new_html_path

            # KB 시세 엑셀 경로
            new_kb = kb_excel_var.get()
            if new_kb:
...
총 추가된 단지: {total_added}개"""
    ...
    def update_all_data(self, _collect_for_unified=None):
        """모든 모니터링 아파트 데이터 갱신"""

        # ★ 전체 통합 선택 시: 각 실제 리스트별로 순차 처리 → 신고가 모아서 마지막에 한 번 팝업
        if self.active_list.get() == self.UNIFIED_LIST_NAME:
            real_lists = [name for name in self.monitored_lists["lists"].keys()
                          if name != self.UNIFIED_LIST_NAME
                          and (self.unified_filter_lists is None or name in self.unified_filter_lists)]
            if not real_lists:
                messagebox.showinfo("알림", "처리할 리스트가 없습니다.")
                return
            prev_list = self.UNIFIED_LIST_NAME
            # 지역별 신고가 수집 - 각 지역 결과를 따로 보관
            regional_results = []  # [(list_name, apt_list), ...]
            for list_name in real_lists:
                collected = []
                self.active_list.set(list_name)
                self.update_all_data(_collect_for_unified=collected)
                if collected:
                    regional_results.append((list_name, collected))
            # 완료 후 통합 리스트로 복원
            ...
            try:
                ...
                if trade_data:
                    ...
                    if new_max_price_trade['price'] > old_max_price:
                        ...
                        # ★ 신고가 발견 즉시 DB 저장 (앱 크래시 대비 - 순위 정보는 나중에 UPDATE)
                        self._save_high_price_log([new_max_info])
        ...
        if new_max_found:
            ...
            # 순위 정보 포함해서 high_price_log 업데이트 (INSERT OR REPLACE)
            self._save_high_price_log(apt_with_new_max)
            ...
            if _collect_for_unified is not None:
    ...
    def build_notification_html(self, apt_list, region_name=None):
            ...
            <div class="compare-modal">
              ...
              <div class="compare-modal-body">
                <div class="compare-settings">
                  <label>가격 차이 범위:</label>
                  <input type="number" id="comparePriceMin" value="1000" placeholder="최소"> 만원
                  <span style="color:#9ca3af">~</span>
                  <input type="number" id="comparePriceMax" value="3000" placeholder="최대"> 만원
                  <button onclick="runCompare()">비교단지 찾기</button>
                </div>
                <div class="compare-count" id="compareCount"></div>
                <div id="compareResults">
                  <div class="compare-empty">
                    <div class="compare-empty-icon">🔍</div>
                    <p>가격 범위를 설정하고 '비교단지 찾기' 버튼을 눌러주세요.<br>
        ...
        function openKakaoMap(element) {{
          let query = element.getAttribute('data-kakao-query');
          if (query) {{
        ...
        function extractCardData(card) {{
          const name = card.querySelector('h3')?.textContent.split(/\s+/)[0] || '알 수 없음';
          const location = card.querySelector('.card-sub')?.textContent.replace('📍', '').replace('🗺️ 지도보기', '').trim() || '';
          const priceText = card.querySelector('.highlight')?.textContent || '0';
          const price = parseInt(priceText.replace(/[^0-9]/g, '')) || 0;
          const area = card.getAttribute('data-area') || '';
          const year = card.getAttribute('data-build-year') || '';
          const kakaoQuery = card.getAttribute('data-kakao-query') || '';

          // 거래 날짜 추출
          const gridDivs = card.querySelectorAll('.grid > div');
          let dateInfo = '';
          for (let i = 0; i < gridDivs.length; i++) {{
            if (gridDivs[i].classList.contains('k') && gridDivs[i].textContent.includes('실거래')) {{
              dateInfo = gridDivs[i+1]?.textContent || '';
              break;
            }}
          }}

          return {{ name, location, price, area, year, dateInfo, kakaoQuery, element: card }};
        }}

        // 비교단지 찾기
        function runCompare() {{
          const minDiff = parseInt(document.getElementById('comparePriceMin').value) || 1000;
          const maxDiff = parseInt(document.getElementById('comparePriceMax').value) || 3000;

          // 현재 보이는 카드만 대상으로
          const visibleCards = Array.from(document.querySelectorAll('.card:not(.hidden)'));
          const cardDataList = visibleCards.map(extractCardData).filter(d => d.price > 0);

          // 가격순 정렬 (고가 → 저가, 내림차순)
          cardDataList.sort((a, b) => b.price - a.price);
          ...
          for (let i = 0; i < cardDataList.length; i++) {{
            ...
            for (let j = i + 1; j < cardDataList.length; j++) {{
              if (used.has(j)) continue;

              const diff = cardDataList[i].price - cardDataList[j].price;
              ...
              // 차이가 maxDiff를 넘어가면 더 이상 볼 필요 없음
              if (diff > maxDiff) break;
            }}
          }}

          // 결과 표시
          renderCompareResults(pairs, minDiff, maxDiff);
        }}

        // 비교 결과 렌더링
        function renderCompareResults(pairs, minDiff, maxDiff) {{
          ...
          if (pairs.length === 0) {{
            ...
            container.innerHTML = `
              <div class="compare-empty">
                <div class="compare-empty-icon">😅</div>
                <p>가격 차이가 ${{minDiff.toLocaleString()}}~${{maxDiff.toLocaleString()}}만원인 비교 단지를 찾지 못했습니다.<br>
                범위를 조정해보세요.</p>
              </div>
            `;
            return;
          ...
          countDiv.innerHTML = `총 <strong>${{pairs.length}}쌍</strong>의 비교단지를 찾았습니다.`;

          let html = '';
          pairs.forEach((pair, idx) => {{
            html += `
              <div class="compare-pair">
                <div class="compare-pair-header">
                  <div class="compare-pair-num">${{idx + 1}}</div>
                  <div class="compare-pair-diff">가격 차이: <strong>${{pair.diff.toLocaleString()}}만원</strong></div>
                </div>
                <div class="compare-items">
                  <div class="compare-item" onclick="openKakaoMapByQuery('${{pair.a.kakaoQuery}}')">
                    <div class="compare-item-name">${{pair.a.name}}</div>
                    <div class="compare-item-loc">📍 ${{pair.a.location}}</div>
                    <div class="compare-item-price">${{pair.a.price.toLocaleString()}}만원</div>
                    <div class="compare-item-meta">${{pair.a.year}}년 | ${{pair.a.area}}㎡ ${{pair.a.dateInfo}}</div>
                  </div>
                  <div class="compare-item" onclick="openKakaoMapByQuery('${{pair.b.kakaoQuery}}')">
                    <div class="compare-item-name">${{pair.b.name}}</div>
                    <div class="compare-item-loc">📍 ${{pair.b.location}}</div>
                    <div class="compare-item-price">${{pair.b.price.toLocaleString()}}만원</div>
                    <div class="compare-item-meta">${{pair.b.year}}년 | ${{pair.b.area}}㎡ ${{pair.b.dateInfo}}</div>
                  </div>
                </div>
              </div>
            `;
        ...
        // 쿼리로 카카오맵 열기
        function openKakaoMapByQuery(query) {{
          if (query) {{
            query = query.replace(/\([^)]*\)/g, '').replace(/\s+/g, ' ').trim();
            const url = 'https://map.kakao.com/?q=' + encodeURIComponent(query);
            window.open(url, '_blank', 'width=1200,height=800');
        ...
        (function(){{
          ...
          const dongToGuMap = {{
            ...
            '산성동': '수정구', '양지동': '수정구', '복정동': '수정구', '신촌동': '수정구', '창곡동': '수정구',
              ...
              if(dongMatch){{
                ...
                if(baseRegion.includes('성남시')){{
                  ...
                  const seongnamDongs = {{
                    ...
                    '양지동':'수정구','복정동':'수정구','신촌동':'수정구','시흥동':'수정구','고등동':'수정구','창곡동':'수정구',
    ...
    def _save_high_price_log(self, apt_list):
        """신고가 발생 단지를 날짜별로 high_price_log 테이블에 저장"""
        try:
            ...
            for apt in apt_list:
                cursor.execute("""
                    INSERT OR REPLACE INTO high_price_log
                        (detected_date, list_name, apt_name, area, sido, sigungu, dong,
                         build_year, old_price, old_date, old_floor, old_dong,
                         new_price, new_date, new_floor, new_dong,
                         region_84_rank, region_59_rank, region_name)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """, (
                    detected_date,
                    current_list,
                    ...
                    apt.get('sido', ''),
                    apt.get('sigungu', ''),
                    apt.get('location_dong', apt.get('dong', '')),
                    apt.get('build_year', ''),
                    apt.get('old_price', 0) or 0,
                    apt.get('old_date', ''),
                    apt.get('old_floor', ''),
                    apt.get('old_dong', ''),
                    apt.get('new_price', 0),
                    ...
                    apt.get('floor', ''),
                    apt.get('dong', ''),
                    apt.get('region_84_rank'),
                    apt.get('region_59_rank'),
                    apt.get('region_name', ''),
                ))
            self.db_conn.commit()
            logging.info(f"[high_price_log] {detected_date} {current_list} {len(apt_list)}건 저장 완료")
        except Exception as e:
            logging.error(f"high_price_log 저장 오류: {e}")
    ...
    def build_area_ranking_html(self, area_type, selected_list_names=None):
        """모니터링 중인 모든 아파트를 대상으로 평형별 가격 순위 HTML 생성
        area_type: '84' 또는 '59'
        selected_list_names: 조회할 리스트 이름 목록 (None이면 현재 active_list)
        """
        import re
        from html import escape
        ...
        try:
            ...
            if selected_list_names and len(selected_list_names) > 0:
                # 선택된 여러 리스트에서 아파트 조회
                real_names = [n for n in selected_list_names if n != self.UNIFIED_LIST_NAME]
                placeholders = ','.join('?' * len(real_names))
                cursor.execute(f"SELECT id FROM monitoring_lists WHERE name IN ({placeholders})",
                               real_names)
                list_ids = [r[0] for r in cursor.fetchall()]
                if not list_ids:
                    return None
                is_multi_list = len(list_ids) > 1
                active_list_id = list_ids[0]
                id_placeholders = ','.join('?' * len(list_ids))
                cursor.execute(f"""
                    ...
                    FROM apartments
                    WHERE list_id IN ({id_placeholders})
                    ORDER BY last_max_price DESC
                """, list_ids)
            elif active_list_name == self.UNIFIED_LIST_NAME:
                # ★ 전체 통합: 모든 리스트에서 조회
                cursor.execute("SELECT id FROM monitoring_lists")
                list_ids = [r[0] for r in cursor.fetchall()]
                if not list_ids:
                    return None
                is_multi_list = True
        ...
        # ========== 중복 단지 제거 (띄어쓰기 무시) ==========
        # 다중 리스트 합산 시: 이름+시군구 키 → 도시가 달라도 같은 이름 허용
        # 단일 리스트 시: 이름만 키 → 같은 리스트 내 중복만 제거
        unique_apts = {}
        for apt in filtered_apts:
            normalized_name = apt['apt_name'].replace(' ', '').replace('-', '').lower()
            if is_multi_list:
                # 도시(sigungu)까지 포함한 키 → 서울 래미안과 부산 래미안은 별개
                dedup_key = (normalized_name, apt.get('sigungu', ''))
            else:
                dedup_key = (normalized_name, '')

            if dedup_key in unique_apts:
                if apt['new_price'] > unique_apts[dedup_key]['new_price']:
                    logging.info(f"[중복 제거] '{unique_apts[dedup_key]['apt_name']}' ({unique_apts[dedup_key]['new_price']:,}만원) → '{apt['apt_name']}' ({apt['new_price']:,}만원) 유지")
                    unique_apts[dedup_key] = apt
                else:
                    logging.info(f"[중복 제거] '{apt['apt_name']}' ({apt['new_price']:,}만원) 제거 (기존: '{unique_apts[dedup_key]['apt_name']}' {unique_apts[dedup_key]['new_price']:,}만원)")
            else:
                unique_apts[dedup_key] = apt
        ...
        # 이전 순위 조회 (단일 리스트일 때만, 가장 최근 2번째 데이터와 비교)
        prev_rankings = {}
        if not is_multi_list:
            try:
                cursor.execute("""
    ...
    def _get_newtrade_filename(self, apt_list=None, prefer_content=False, list_name_hint=None):
        ...
        # ★ 전체 통합 or 매핑 없는 리스트: apt_list의 sido로 지역 판별 (부분 일치)
        if apt_list:
            sido_set = set()
            for apt in apt_list:
                s = apt.get("sido", "")
            ...
            # sido 비어있을 때: sigungu 이름으로 지역 판별 (구 단위 고유 이름 매칭)
            sigungu_set = {apt.get("sigungu", "") for apt in apt_list if apt.get("sigungu")}
            if sigungu_set:
                BUSAN_ONLY   = {"해운대구", "수영구", "동래구", "영도구", "연제구",
                                "사상구", "기장군", "부산진구", "사하구", "금정구"}
        ...
        # sido/sigungu 매칭 실패 시: list_name_hint (히스토리 저장 당시 리스트명) 우선 체크
        if list_name_hint and list_name_hint in name_map:
            return name_map[list_name_hint]
        # 현재 active_list name_map 체크
        if list_name in name_map:
            return name_map[list_name]
        # 최종 폴백: list_name_hint 기반 (name_map 미매칭 시)
        effective_name = list_name_hint or list_name
        safe_name = effective_name.replace(" ", "_").replace("★", "").strip("_") if effective_name else "기본"
        if not safe_name:
            safe_name = "기본"
        return f"{safe_name}신고가.html"

    def export_notification_html(self, apt_list, *, ask_path=False, silent=False, fixed_filename=None, region_name=None):
        """신고가 알림 HTML 파일 저장 및 열기
        - newtrade 폴더에 리스트별 고정 파일명으로 저장
        - 기존 파일이 있으면 backup 폴더로 이동 후 새 파일 생성
        ...
        import shutil
        try:
            html_str = self.build_notification_html(apt_list, region_name=region_name)

            if ask_path:
                # 수동 저장: 사용자가 경로 선택
                initdir = os.path.join(self.html_download_path, "newtrade")
                os.makedirs(initdir, exist_ok=True)
                from tkinter import filedialog
                default = fixed_filename or self._get_newtrade_filename()
                filepath = filedialog.asksaveasfilename(
                ...
                # 자동 저장: html_download_path 폴더에 고정 파일명
                ...
                if os.path.exists(filepath):
                    backup_dir = os.path.join(newtrade_dir, "backup")
                    os.makedirs(backup_dir, exist_ok=True)
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    name_only = os.path.splitext(filename)[0]
                    backup_name = f"{name_only}_backup_{ts}.html"
                    backup_path = os.path.join(backup_dir, backup_name)
                    shutil.move(filepath, backup_path)
                    logging.info(f"[백업] 기존 파일 이동: {backup_path}")
    ...
    def export_area_ranking_html(self, area_type):
        """평형별 가격 순위 HTML 다운로드
        area_type: '84' 또는 '59'
        현재 active_list 기준으로 생성 (★ 전체 통합 선택 시 지역 선택 다이얼로그 표시)
        """
        try:
            active_list_name = self.active_list.get()

            # ★ 전체 통합 선택 시: 지역 선택 다이얼로그 표시
            if active_list_name == self.UNIFIED_LIST_NAME:
                chosen = self._show_unified_region_select_dialog(f"{area_type}㎡ 순위 HTML - 지역 선택")
                if chosen == "cancel":
                    return None
                if chosen is not None:
                    # 특정 지역 선택: 임시로 해당 리스트로 전환 후 재귀 생성
                    self.active_list.set(chosen)
                    result = self.export_area_ranking_html(area_type)
                    self.active_list.set(self.UNIFIED_LIST_NAME)
                    return result
                # chosen is None → 전체 합산으로 계속
            ...
            # 저장 경로
            reports_dir = os.path.join(self.html_download_path, "newtrade")
            os.makedirs(reports_dir, exist_ok=True)

            # 현재 active_list로 파일명 결정
            active_list_name = self.active_list.get()
            if active_list_name == self.UNIFIED_LIST_NAME:
                region_prefix = "전체"
            elif "서울" in active_list_name or "수도권" in active_list_name:
                region_prefix = "서울&수도권"
            elif "대구" in active_list_name:
                region_prefix = "대구"
            elif "부산" in active_list_name:
                region_prefix = "부산"
            else:
                region_prefix = active_list_name

            filename = f"{region_prefix} {area_type} 순위.html"
    ...
    def export_trade_volume_ranking_html(self):
        ...
        def on_ok():
            try:
                ...
                # 저장 경로
                reports_dir = os.path.join(self.html_download_path, "reports")
                os.makedirs(reports_dir, exist_ok=True)
    ...
    REGION_TIERS = {
        1: {  # 코랄레드 - 최상위
            'color': '#E74C3C',
            'name': '1티어',
            'regions': ['서울 강남구', '서울 서초구', '과천시', '서울 송파구', '서울 용산구']
        },
        2: {  # 캐럿오렌지
            'color': '#E67E22',
            'name': '2티어',
            'regions': ['서울 성동구', '서울 양천구', '서울 마포구', '서울 광진구', '서울 강동구',
                       '성남시 분당구', '서울 영등포구', '서울 동작구', '서울 중구', '서울 종로구']
        },
        3: {  # 선플라워
            'color': '#F1C40F',
            'name': '3티어',
            'regions': ['성남시 수정구', '서울 서대문구', '서울 강서구', '서울 동대문구', '하남시']
        },
        4: {  # 에메랄드
            'color': '#2ECC71',
            'name': '4티어',
            'regions': ['서울 성북구', '서울 은평구', '서울 관악구', '성남시 중원구', '광명시', '안양시 동안구']
        },
        5: {  # 피터리버
            'color': '#3498DB',
            'name': '5티어',
            'regions': ['서울 구로구', '서울 노원구', '서울 중랑구', '구리시',
                       '용인시 수지구', '수원시 영통구', '서울 금천구', '서울 강북구', '의왕시']
        },
        6: {  # 아메시스트
            'color': '#9B59B6',
            'name': '6티어',
            'regions': ['수원시 팔달구', '서울 도봉구', '안양시 만안구', '부천시 원미구', '화성시',
                       '고양시 덕양구', '수원시 장안구', '부천시 소사구', '군포시', '인천 연수구',
                       '안산시 단원구', '용인시 기흥구', '고양시 일산동구', '수원시 권선구',
                       '남양주시', '안산시 상록구', '인천 서구', '김포시', '시흥시']
        },
        7: {  # 위스테리아
            'color': '#8E44AD',
            'name': '7티어',
            'regions': ['고양시 일산서구', '인천 부평구', '부천시 오정구', '광주시', '의정부시',
                       '용인시 처인구', '인천 남동구', '파주시', '인천 중구', '오산시',
                       '인천 미추홀구', '인천 계양구', '평택시', '양주시', '인천 동구',
                       '이천시', '안성시', '동두천시']
        }
    }
    ...
    def get_region_tier(self, sigungu):
        ...
        for tier, data in self.REGION_TIERS.items():
            for region in data['regions']:
                # 정확히 일치하거나 포함 관계 체크
                if region in normalized or normalized in region:
                    return tier
                # 부분 매칭 (서울 강남구 <-> 강남구)
                region_parts = region.split()
                if len(region_parts) > 1 and region_parts[-1] in normalized:
                    return tier
        return None  # 티어에 없는 지역

    def calculate_fear_greed_index(self, all_trades):
        """공포 탐욕지수 계산
        ...
        # 티어별 거래량 및 신고가 집계
        tier_trades_2w = defaultdict(int)  # 최근 2주
        tier_trades_4w = defaultdict(int)  # 최근 4주
        tier_new_highs_4w = defaultdict(int)  # 최근 4주 신고가 개수

        for trade in all_trades:
            sigungu = trade.get('sigungu', '')
            tier = self.get_region_tier(sigungu)
            if tier is None:
                continue

            trade_date_str = trade.get('date', '')
            is_new_high = trade.get('is_new_high', False)
            ...
            # 4주 이내 거래
            if trade_date >= four_weeks_ago:
                tier_trades_4w[tier] += 1

                # 신고가인 경우
                if is_new_high:
                    tier_new_highs_4w[tier] += 1

                # 2주 이내 거래
                if trade_date >= two_weeks_ago:
                    tier_trades_2w[tier] += 1

        # 티어별 지수 계산
        tier_indices = {}
        for tier in range(1, 8):
            trades_2w = tier_trades_2w.get(tier, 0)
            trades_4w = tier_trades_4w.get(tier, 0)
            new_highs_4w = tier_new_highs_4w.get(tier, 0)

            # 비율 계산: 2주 거래량 / 4주 전체 거래량 × 100
            if trades_4w > 0:
                ratio = (trades_2w / trades_4w) * 100
            else:
                ratio = 50  # 데이터 없으면 중립 (50%)
            ...
            tier_indices[tier] = {
                ...
                'ratio': ratio,
                'color': self.REGION_TIERS[tier]['color'],
                'name': self.REGION_TIERS[tier]['name']
            }
        ...
        weighted_sum = 0
        for tier, data in tier_indices.items():
            weighted_sum += data['ratio'] * weights[tier]

        overall_index = weighted_sum / total_weight if total_weight > 0 else 50
        normalized_index = min(100, max(0, overall_index / 2))

        return {
            'overall': normalized_index,
            'tiers': tier_indices
        }

    def build_fear_greed_html(self, hotplace_region='', hotplace_rate='', kb_analysis=None):
        """공포 탐욕지수 HTML 생성 (가격대별 분위와 동일한 데이터 수집 방식)"""
        from html import escape

        # 거래 데이터 수집 (가격대별 분위와 동일)
        all_trades = []
        today = datetime.now()
        from dateutil.relativedelta import relativedelta
        from datetime import timedelta
        ...
        seen_trades = set()
        # 서울/경기/인천 각각 지역별 4주 거래량
        seoul_trades_4w = defaultdict(int)
        gyeonggi_trades_4w = defaultdict(int)
        incheon_trades_4w = defaultdict(int)

        for apt_info in self.monitored_apts:
            apt_name = apt_info.get('apt_name', '')
            sigungu = apt_info.get('sigungu', '')
            dong = apt_info.get('dong', '')
            sido = apt_info.get('sido', '')  # 시도 필드 추가
            ...
            for trade in trade_data:
                try:
                    ...
                    # 시도별 분류 (서울/경기/인천 수도권만 포함, 그 외 지역 제외)
                    ...
                    if '서울' in sido_lower or '서울' in sigungu_lower:
                        ...
                        sido_type = 2
                    elif '경기' in sido_lower or '경기' in sigungu_lower:
                        sido_type = 1
                    else:
                        continue  # 수도권(서울·경기·인천) 외 지역 제외
    ...
    <div class="kb-analysis-section">
      ...
      <div class="kb-analysis-grid">
        <div class="kb-card weekly">
          ...
          {render_items(kb_analysis.get('weekly_top3', []))}
        </div>
        <div class="kb-card">
          <div class="kb-card-label">📅 3개월 누적 상승 TOP3</div>
          {render_items(kb_analysis.get('month3_top3', []))}
        </div>
        <div class="kb-card">
          <div class="kb-card-label">📅 6개월 누적 상승 TOP3</div>
          {render_items(kb_analysis.get('month6_top3', []))}
        </div>
        <div class="kb-card yearly">
          <div class="kb-card-label">📅 1년 누적 상승 TOP3</div>
...
<style>
  ...
  *{{box-sizing:border-box; margin:0; padding:0;}}
  ...
  .main-title{{margin:0; font-size:clamp(24px, 3.5vw, 32px); letter-spacing:-.3px; font-weight:800}}
  ...
  .block-card::before{{
    ...
    position:absolute;
    top:0; left:0; right:0; bottom:0;
    background:linear-gradient(135deg, rgba(76,175,80,.1) 0%, rgba(76,175,80,.02) 100%);
    opacity:0;
    transition:opacity .3s ease;
  }}
  .block-card:hover{{
    transform:translateY(-8px);
    box-shadow:0 12px 40px rgba(76,175,80,.2);
  ...
  .kb-card.weekly .kb-card-item .value {{
    color: #FF6B6B;
  }}
  ...
  .watermark-text span {{
    font-size:16px; color:rgba(255,255,255,0.03);
    padding:30px 50px; white-space:nowrap;
    user-select:none; -webkit-user-select:none;
    font-weight:600;
  }}
  ...
  .gauge-bg {{
    ...
    background:conic-gradient(
      from 270deg,
      #E74C3C 0deg 25.7deg,      /* 1티어 코랄레드 */
      #E67E22 25.7deg 51.4deg,   /* 2티어 캐럿오렌지 */
      #F1C40F 51.4deg 77.1deg,   /* 3티어 선플라워 */
      #2ECC71 77.1deg 102.8deg,  /* 4티어 에메랄드 */
      #3498DB 102.8deg 128.5deg, /* 5티어 피터리버 */
      #9B59B6 128.5deg 154.2deg, /* 6티어 아메시스트 */
      #8E44AD 154.2deg 180deg    /* 7티어 위스테리아 */
    );
...
<header class="main-header">
  <div class="wrap">
    <div>
      <h1 class="main-title">부태리의 부동산 신고가 & 단지랭킹</h1>
      <div class="sub">전국 아파트 신고가 정보와 지역별 단지 순위를 한눈에</div>
    </div>
  </div>
</header>
...
<main class="wrap">
  <div class="block-grid">
    <!-- 신고가 블럭카드 -->
    <div class="block-card" onclick="location.href='신고가.html'">
      <span class="block-icon">📈</span>
      <h2 class="block-title">신고가</h2>
      <p class="block-desc">서울·수도권, 대구, 부산 지역별 아파트 신고가 정보를 확인하세요</p>
    </div>

    <!-- 단지랭킹 블럭카드 -->
    <div class="block-card" onclick="location.href='단지랭킹.html'">
      <span class="block-icon">🏆</span>
      <h2 class="block-title">단지랭킹</h2>
      <p class="block-desc">84㎡, 59㎡ 타입별 지역 최고가 단지 순위를 확인하세요</p>
    </div>

    <!-- 단지 매전차트 분석 블럭카드 -->
    <div class="block-card" onclick="location.href='단지 매전차트.html'">
      <span class="block-icon">📊</span>
    ...
    <div class="gauge-section">
      <div class="gauge-container">
        ...
        <div class="gauge-labels">
          <span style="background:#E74C3C;">1</span>
          <span style="background:#E67E22;">2</span>
          <span style="background:#F1C40F;color:#000;">3</span>
          <span style="background:#2ECC71;">4</span>
          <span style="background:#3498DB;">5</span>
          <span style="background:#9B59B6;">6</span>
          <span style="background:#8E44AD;">7</span>
        </div>
        <!-- 지수 표시 -->
...
<main class="wrap">
  ...
  <div class="tab-panel" id="panel1">
    <div class="chart-section">
      ...
      <div class="chart-wrap" id="qtlChartWrap">
        <canvas id="qtlChart" style="max-height:600px;"></canvas>
      </div>
      <div class="table-section">
        <div class="table-header">
...
function qtlFilterChips() {{
  var q = (document.getElementById('qtlSearch').value || '').trim().toLowerCase();
  document.querySelectorAll('#qtlChipArea .py-chip').forEach(function(chip) {{
    chip.classList.toggle('hidden', !!(q && !chip.textContent.toLowerCase().includes(q)));
  }});
}}

function qtlRenderChips() {{
  ...
  QTL_STATS.forEach(function(d) {{
    ...
    chip.addEventListener('click', function() {{
      if (qtlSelectedSet.has(d.sigungu)) {{ qtlSelectedSet.delete(d.sigungu); chip.classList.remove('selected'); }}
      else {{ qtlSelectedSet.add(d.sigungu); chip.classList.add('selected'); }}
      qtlUpdateSelCount();
      qtlDrawChart();
    }});
    area.appendChild(chip);
...
function qtlUpdateSelCount() {{
  var el = document.getElementById('qtlSelCount');
  if (el) el.textContent = qtlSelectedSet.size;
}}

function qtlDrawChart() {{
  var filtered = QTL_STATS.filter(function(d){{ return qtlSelectedSet.has(d.sigungu); }});
  var canvas = document.getElementById('qtlChart');

  if (filtered.length === 0) {{
    if (qtlChartInstance) {{ qtlChartInstance.destroy(); qtlChartInstance = null; }}
    canvas.style.display = 'none';
    document.getElementById('qtlTableBody').innerHTML = '';
    return;
  }}
  canvas.style.display = '';

  var canvasH = Math.max(320, Math.min(filtered.length * 40 + 60, 700));
  document.getElementById('qtlChartWrap').style.height = canvasH + 'px';
  canvas.style.maxHeight = 'none';

  var labels = filtered.map(function(d){{return d.sigungu;}});
  if (qtlChartInstance) {{ qtlChartInstance.destroy(); qtlChartInstance = null; }}
  qtlChartInstance = new Chart(canvas.getContext('2d'), {{
    type: 'bar',
    data: {{
      labels: labels,
      datasets: [
        {{ label:'Q1 (하위25%)', data:filtered.map(function(d){{return d.q1;}}),
           backgroundColor:'rgba(33,150,243,0.7)', borderRadius:3, borderWidth:0 }},
        {{ label:'중위 (50%)',   data:filtered.map(function(d){{return d.med;}}),
           backgroundColor:'rgba(76,175,80,0.8)',  borderRadius:3, borderWidth:0 }},
        {{ label:'Q3 (상위25%)', data:filtered.map(function(d){{return d.q3;}}),
           backgroundColor:'rgba(255,152,0,0.8)',  borderRadius:3, borderWidth:0 }},
        {{ label:'최고가',       data:filtered.map(function(d){{return d.max;}}),
           backgroundColor:'rgba(244,67,54,0.75)', borderRadius:3, borderWidth:0 }}
      ]
    }},
    options: {{
      indexAxis: 'y',
      ...
      plugins: {{
        legend: {{ display:true, labels:{{color:'rgba(255,255,255,0.75)',boxWidth:14,font:{{size:12}}}} }},
        tooltip: {{ callbacks: {{ label: function(c) {{
          return ' ' + c.dataset.label + ': ' + Math.round(c.parsed.x).toLocaleString() + ' 만원';
        }} }} }}
      }},
      scales: {{
        x: {{ ticks:{{color:'rgba(255,255,255,0.6)',callback:function(v){{return (v/10000).toFixed(1)+'억';}}}},
              grid:{{color:'rgba(255,255,255,0.07)'}} }},
  ...
  document.getElementById('qtlTableBody').innerHTML = filtered.map(function(d, i) {{
...
function renderQtlChart() {{
  if (!QTL_STATS || QTL_STATS.length === 0) {{
    document.getElementById('panel1').querySelector('#qtlChartWrap').innerHTML =
  ...
  renderQtlChart();
    ...
    def export_fear_greed_html(self):
        ...
        try:
            ...
            messagebox.showinfo("저장 완료", f"부동산 최신 근황 HTML 저장: {filepath}")
            logging.info(f"[부동산 최신 근황] HTML 저장: {filepath}")
            return filepath

        except Exception as e:
            logging.error(f"부동산 최신 근황 HTML 생성 중 오류: {str(e)}")
            import traceback
            logging.error(traceback.format_exc())
            messagebox.showerror("오류", f"HTML 생성 실패: {str(e)}")
            return None
    ...
    def export_price_distribution_html(self):
        ...
        try:
            ...
            # ★ 전체 통합 선택 시: 지역 선택 다이얼로그 표시
            if active_list_name == self.UNIFIED_LIST_NAME:
                chosen = self._show_unified_region_select_dialog("가격대별 분위 HTML - 지역 선택")
                if chosen == "cancel":
                    return
                if chosen is not None:
                    # 특정 지역 선택: 임시로 해당 리스트로 전환 후 재귀 생성
                    self.active_list.set(chosen)
                    self.export_price_distribution_html()
                    self.active_list.set(self.UNIFIED_LIST_NAME)
                    return
                # chosen is None → 전체 합산으로 계속

            # 서울수도권 단독 선택 시에만 세부 지역 필터 제공
            region_filter = None
            is_seoul_sudogwon = (active_list_name != self.UNIFIED_LIST_NAME and
                                 "서울" in active_list_name and "수도권" in active_list_name)
            ...
            if region_filter:
                ...
                region_name = "전체"
            elif "서울" in active_list_name and "수도권" in active_list_name:
                region_name = "서울&수도권"
            elif "대구" in active_list_name:
                region_name = "대구"
            elif "부산" in active_list_name:
                ...
                region_name = active_list_name.replace(" ", "_")

            # html_download_path/newtrade 에 저장
    ...
    def _show_list_selection_dialog(self, title="지역 선택"):
        ...
        반환값: 선택된 리스트 이름 목록 (취소 시 None)
        """
        # DB에서 모든 리스트 이름 조회
        try:
            cursor = self.db_conn.cursor()
            cursor.execute("SELECT name FROM monitoring_lists ORDER BY id")
            all_list_names = [r[0] for r in cursor.fetchall()]
        except Exception as e:
            logging.error(f"모니터링 리스트 조회 오류: {str(e)}")
            all_list_names = list(self.monitored_lists.get("lists", {}).keys())
        ...
        # 화면 중앙 배치
        dialog.update_idletasks()
        dlg_w, dlg_h = 320, 80 + len(all_list_names) * 36 + 100
        ...
        ttk.Label(dialog, text=title, font=("맑은 고딕", 11, "bold")).pack(pady=(15, 5))
        ttk.Label(dialog, text="HTML로 합쳐서 보고 싶은 지역을 선택하세요",
                  font=("맑은 고딕", 9)).pack(pady=(0, 10))

        # 체크박스 변수 생성
        check_vars = {}
        for name in all_list_names:
            var = tk.BooleanVar(value=True)  # 기본: 전체 선택
            check_vars[name] = var
        ...
        for name in all_list_names:
            ttk.Checkbutton(chk_frame, text=name, variable=check_vars[name]).pack(anchor="w", pady=3)
        ...
        ttk.Button(btn_frame, text="전체 선택", command=select_all, width=12).pack(side="left", padx=3)
        ttk.Button(btn_frame, text="전체 해제", command=deselect_all, width=12).pack(side="left", padx=3)

        def on_confirm():
            selected = [name for name, var in check_vars.items() if var.get()]
            if not selected:
                messagebox.showwarning("알림", "최소 1개 이상의 지역을 선택하세요.", parent=dialog)
                return
        ...
        action_frame.pack(pady=(5, 15), fill="x", padx=20)
        ttk.Button(action_frame, text="✅ 확인", command=on_confirm,
                   style='Accent.TButton', width=14).pack(side="left", padx=3)
        ttk.Button(action_frame, text="취소", command=on_cancel, width=8).pack(side="left", padx=3)

        dialog.wait_window()
        return result["value"]

    def _get_region_prefix_from_lists(self, selected_list_names):
        """선택된 리스트 이름 목록으로 파일명용 지역 prefix 반환"""
        if not selected_list_names:
            return self.active_list.get()

        # 리스트별 지역 레이블 결정
        labels = []
        for name in selected_list_names:
            if "서울" in name or "수도권" in name:
                label = "서울&수도권"
            elif "대구" in name:
                label = "대구"
            elif "부산" in name:
    ...
    def _show_unified_region_select_dialog(self, title="지역 선택"):
        """★ 전체 통합 리스트에서 HTML 생성 시 지역 선택 다이얼로그.
        반환값:
          - 특정 리스트 이름 (str): 해당 지역만 생성
          - None: 전체 합산으로 생성
          - "cancel": 취소
        """
        real_lists = [name for name in self.monitored_lists["lists"].keys()
        ...
        if not real_lists:
            return None  # 리스트 없으면 전체 합산으로 처리
        ...
        ttk.Label(dialog, text="HTML 생성 지역 선택",
                  font=("맑은 고딕", 12, "bold")).pack(pady=(15, 3))
        ttk.Label(dialog, text="원하는 지역을 선택하거나 전체 합산을 선택하세요",
                  font=("맑은 고딕", 9)).pack(pady=(0, 10))
        ...
        def select_list(list_name):
            result["value"] = list_name  # None = 전체 합산
            dialog.destroy()
        ...
        for list_name in real_lists:
            if "서울" in list_name or "수도권" in list_name:
                ...
                display = list_name
            ttk.Button(btn_frame, text=f"📍 {display}",
                       command=lambda ln=list_name: select_list(ln),
                       width=28).pack(pady=4)

        ttk.Separator(btn_frame, orient="horizontal").pack(fill="x", pady=8)

        # 전체 합산 버튼 (None 전달)
        ttk.Button(btn_frame, text="📊 전체 합산 (모든 지역)",
                   command=lambda: select_list(None),
                   width=28).pack(pady=4)

        ttk.Separator(dialog, orient="horizontal").pack(fill="x", padx=20, pady=5)
        ttk.Button(dialog, text="취소",
                   command=lambda: select_list("cancel"),
    ...
    def build_price_distribution_html(self, region_filter=None):
        """모니터링 중인 아파트 리스트의 거래 데이터를 사용하여 HTML 생성
        현재 active_list 기준 (★ 전체 통합 선택 시 모든 리스트 합산)

        Args:
            region_filter: 지역 필터 (None=전체, "서울", "경기", "인천") - 서울수도권 단독 선택 시
        """
        from html import escape

        # 조회 대상 아파트 목록 결정 (monitored_apts 프로퍼티가 통합 리스트 처리)
        target_apts = self.monitored_apts
        active_name = self.active_list.get() if hasattr(self, 'active_list') else "모니터링 리스트"
        list_label = "전체 통합" if active_name == self.UNIFIED_LIST_NAME else active_name
        ...
        logging.info(f"[가격대별 분위] 모니터링 중인 아파트 수: {len(target_apts)}개")
        ...
        try:
            ...
            if region_filter and target_apts:
                sample_apt = target_apts[0]
            ...
            for apt_info in target_apts:
                ...
                # apt_info['last_max_price']가 실제 신고가임
                last_max_price = apt_info.get('last_max_price', 0)
                ...
                for trade in trade_data:
                    try:
                        ...
                        # 신고가 여부 판별 (모니터링 아파트의 last_max_price와 같으면 신고가)
                        is_highest = (deal_amount == last_max_price) and last_max_price > 0
        ...
        <style>
          * {{ margin:0; padding:0; box-sizing:border-box; }}
          ...
          .price-cat-table th:first-child {{
            border-radius:10px 0 0 0;
          }}
          .price-cat-table th:last-child {{
          ...
          .price-cat-table tr:last-child td {{
            border-bottom:none;
          }}
          .price-cat-table tr:nth-child(even) {{
            background:#fff;
          }}
          .price-cat-table tr:nth-child(odd) {{
            background:#f0f0f5;
          }}
          .price-cat-table tr:hover {{
            background:#e8e8f0;
          }}
          .price-cat-table .cat-label {{
            font-weight:700;
            color:#333;
            text-align:left;
            padding-left:20px;
          }}
          .price-cat-table .change-up {{
            color:#dc2626;
            font-weight:700;
          }}
          .price-cat-table .change-down {{
            color:#2563eb;
            font-weight:700;
          }}
          .price-cat-table .change-zero {{
            color:#888;
          }}
          .price-cat-table .pct-bar {{
            display:inline-block;
            height:8px;
            border-radius:4px;
            margin-left:8px;
        ...
        <div class="container">
          ...
          <div class="price-category-section">
            ...
            <table class="price-cat-table">
              <thead>
                <tr>
                  <th style="width:18%;">가격대</th>
                  <th style="width:15%;" id="catP1Title">과거 기간</th>
                  <th style="width:12%;">비율</th>
                  <th style="width:15%;" id="catP2Title">최신 기간</th>
                  <th style="width:12%;">비율</th>
                  <th style="width:14%;">건수 변화</th>
                  <th style="width:14%;">비율 변화</th>
                </tr>
              </thead>
        ...
        <script>
          ...
          function updateChart() {{
            ...
            if (p1Start <= p2Start) {{
              updatePriceCategoryTable(period1Data, period2Data, period1Total, period2Total, period1Label, period2Label);
            }} else {{
              updatePriceCategoryTable(period2Data, period1Data, period2Total, period1Total, period2Label, period1Label);
            }}
          ...
          function updatePriceCategoryTable(oldData, newData, oldTotal, newTotal, oldLabel, newLabel) {{
            ...
            const oldCounts = countByCategory(oldData);
            const newCounts = countByCategory(newData);

            // 헤더: 과거(왼쪽) → 최신(오른쪽)
            const catP1Title = document.getElementById('catP1Title');
            const catP2Title = document.getElementById('catP2Title');
            ...
            for (let i = 0; i < categories.length; i++) {{
              ...
              const cOld = oldCounts[i];
              const cNew = newCounts[i];
              const pctOld = oldTotal > 0 ? (cOld / oldTotal * 100) : 0;
              const pctNew = newTotal > 0 ? (cNew / newTotal * 100) : 0;

              // 변화량 = 최신 - 과거
              const countDiff = cNew - cOld;
              const countDiffStr = countDiff > 0 ? '+' + countDiff : '' + countDiff;
              const countClass = countDiff > 0 ? 'change-up' : (countDiff < 0 ? 'change-down' : 'change-zero');

              const pctDiff = pctNew - pctOld;
              const pctDiffStr = pctDiff > 0 ? '+' + pctDiff.toFixed(1) + '%p' : pctDiff.toFixed(1) + '%p';
              const pctClass = pctDiff > 0 ? 'change-up' : (pctDiff < 0 ? 'change-down' : 'change-zero');

              const barWidthOld = Math.min(pctOld, 100);
              const barWidthNew = Math.min(pctNew, 100);

              html += `<tr>
                <td class="cat-label">${{cat.label}}</td>
                <td style="font-weight:600;">${{cOld.toLocaleString()}}건</td>
                <td>${{pctOld.toFixed(1)}}%<span class="pct-bar" style="width:${{barWidthOld}}px;background:${{barColors[i]}};opacity:0.4;"></span></td>
                <td style="font-weight:600;">${{cNew.toLocaleString()}}건</td>
                <td>${{pctNew.toFixed(1)}}%<span class="pct-bar" style="width:${{barWidthNew}}px;background:${{barColors[i]}};opacity:0.7;"></span></td>
                <td class="${{countClass}}">${{countDiffStr}}건</td>
                <td class="${{pctClass}}">${{pctDiffStr}}</td>
              </tr>`;
            }}

            // 합계: 변화량 = 최신 - 과거
            const totalCountDiff = newTotal - oldTotal;
            const totalCountStr = totalCountDiff > 0 ? '+' + totalCountDiff : '' + totalCountDiff;
            const totalCountClass = totalCountDiff > 0 ? 'change-up' : (totalCountDiff < 0 ? 'change-down' : 'change-zero');

            html += `<tr class="price-cat-total">
              <td class="cat-label">합계</td>
              <td style="font-weight:700;">${{oldTotal.toLocaleString()}}건</td>
              <td>100%</td>
              <td style="font-weight:700;">${{newTotal.toLocaleString()}}건</td>
              <td>100%</td>
              <td class="${{totalCountClass}}">${{totalCountStr}}건</td>
              <td>-</td>
            </tr>`;

            document.getElementById('priceCatBody').innerHTML = html;
          }}
    ...
    def show_new_max_notification(self, apt_list, list_name_hint=None):
        ...
        try:
            ...
            if not is_duplicate:
                # 메모리에 추가 (list_name 저장: 히스토리 재생 시 파일명 결정에 사용)
                saved_list_name = self.active_list.get() if hasattr(self, 'active_list') else ''
                notification_data = {'timestamp': current_time, 'apt_list': apt_list.copy(), 'list_name': saved_list_name}
                self.notifications_history.append(notification_data)
        ...
        def update_page():
            ...
            for i, apt in enumerate(current_apts):
                ...
                apt_name_label = tk.Label(apt_header, text=f"🏢 {apt['apt_name']}", font=scaled_font(get_mac_font(), apt_name_size, 'bold'), bg=colors['card_bg'], fg=colors['text_primary'], anchor='w')
                ...
                if location_text:
                    ...
                    location_label = tk.Label(apt_header, text=location_text, font=scaled_font(get_mac_font(), location_size), bg=colors['card_bg'], fg=colors['text_secondary'], anchor='w')
                    location_label.pack(anchor='w', pady=(1, 0))
                
                area_size = max(8, int(10 * scale_factor))
                area_label = tk.Label(apt_header, text=f"📐 {apt['area']}㎡", font=scaled_font(get_mac_font(), area_size), bg=colors['card_bg'], fg=colors['text_secondary'], anchor='w')
                ...
                if old_price > 0:
                    ...
                    tk.Label(old_price_frame, text="이전 최고가",
                             font=scaled_font(get_mac_font(), old_label_size),
                ...
                tk.Label(new_price_frame, text="📈 신고가", font=scaled_font(get_mac_font(), new_label_size, 'bold'), bg=colors['card_bg'], fg=colors['error']).pack(side='left')
                ...
                tk.Label(new_price_frame, text=f"{new_price:,}만원", font=scaled_font(get_mac_font(), new_price_size, 'bold'), bg=colors['card_bg'], fg=colors['error']).pack(side='right')
                if old_price > 0:
                    ...
                    increase_label = tk.Label(highlight_content, text=f"🔥 {increase:,}만원 상승 (+{increase_percent:.1f}%)", font=scaled_font(get_mac_font(), increase_size, 'bold'), bg=colors['error_light'], fg='#C62828')
                    ...
                    detail_label = tk.Label(highlight_content, text=detail_info, font=scaled_font(get_mac_font(), detail_size), bg=colors['error_light'], fg=colors['text_secondary'])
        ...
        if total_pages > 1:
            ...
            def go_prev():
                ...
                if current_page > 0:
                    current_page -= 1
                    update_page()
            prev_button = tk.Button(nav_frame, text="◀", font=scaled_font(get_mac_font(), 12, 'bold'), bg=colors['primary'], fg='white', activebackground=colors['primary_dark'], relief='flat', bd=0, width=4, height=1, cursor='hand2', command=go_prev)
            prev_button.pack(side='left')
            page_display = tk.Label(nav_frame, text="", font=scaled_font(get_mac_font(), 11, 'bold'), bg=colors['background'], fg=colors['text_primary'])
            ...
            def go_next():
                ...
                if current_page < total_pages - 1:
                    current_page += 1
                    update_page()
            next_button = tk.Button(nav_frame, text="▶", font=scaled_font(get_mac_font(), 12, 'bold'), bg=colors['primary'], fg='white', activebackground=colors['primary_dark'], relief='flat', bd=0, width=4, height=1, cursor='hand2', command=go_next)
        ...
        def capture_screen():
            try:
                ...
                if IS_MAC:
                    try:
                        dpi_scale = notification_window.winfo_fpixels('1i') / 72.0
                    except:
                        dpi_scale = 2  # Retina 기본값
                ...
                try:
                    if IS_MAC and ImageGrab is None:
                        # macOS에서 ImageGrab 없으면 screencapture 명령 사용
                        capture_folder = os.path.join(self.download_path, "captures")
                        os.makedirs(capture_folder, exist_ok=True)
                        timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
                        page_info_str = f"_page{current_page + 1}of{total_pages}" if total_pages > 1 else ""
                        fn = f"부태리신고가_{timestamp_str}{page_info_str}.png"
                        fp = os.path.join(capture_folder, fn)
                        subprocess.run(['screencapture', '-R', f'{x},{capture_y},{width},{capture_bottom - capture_y}', fp])
                        capture_button.config(text="✓", fg=colors['success'])
                        notification_window.after(3000, lambda: capture_button.config(text="📸", fg='white'))
                        print(f"📸 화면 캡처 성공 (screencapture): {fp}")
                        return
                    # macOS Retina 대응: bbox 좌표에 dpi_scale 적용
                    if IS_MAC and dpi_scale > 1:
                        bbox = (int(x * dpi_scale), int(capture_y * dpi_scale),
                                int((x + width) * dpi_scale), int(capture_bottom * dpi_scale))
                    else:
        ...
        confirm_button = tk.Button(
            button_frame,
            text="확인",
            font=scaled_font(get_mac_font(), button_size, 'bold'),
        ...
        html_button = tk.Button(
            ...
            command=lambda fn=_fixed_filename, rn=_region_display: self.export_notification_html(apt_list, ask_path=False, silent=False, fixed_filename=fn, region_name=rn)
        )
        ...
        return notification_window  # 호출자가 wait_window()로 대기할 수 있도록 반환

    def _show_regional_html_dialog(self, regional_results):
        """지역별 신고가 HTML 생성 다이얼로그 (데이터 갱신 후 또는 버튼으로 호출)"""
        prev_list = self.active_list.get()

        dialog = tk.Toplevel(self.root)
        dialog.title("신고가 HTML 생성")
        dialog.resizable(False, False)
        dialog.transient(self.root)
        dialog.grab_set()

        dlg_w = 340
        dlg_h = 160 + len(regional_results) * 52 + 90
        x = (dialog.winfo_screenwidth() - dlg_w) // 2
        y = (dialog.winfo_screenheight() - dlg_h) // 2
        dialog.geometry(f"{dlg_w}x{dlg_h}+{x}+{y}")

        ttk.Label(dialog, text="신고가 HTML 생성",
                  font=("맑은 고딕", 12, "bold")).pack(pady=(15, 3))
        ttk.Label(dialog, text="HTML 파일을 생성할 지역을 선택하세요",
                  font=("맑은 고딕", 9)).pack(pady=(0, 10))
        ...
        for list_name, apt_list in regional_results:
            if "서울" in list_name or "수도권" in list_name:
                ...
                display = "부산"
            elif "대전" in list_name:
                display = "대전"
            else:
                display = list_name
            count = len(apt_list)
            ttk.Button(btn_frame,
                       text=f"📍 {display}  ({count}개 신고가) HTML 생성",
                       command=lambda ln=list_name, al=apt_list: gen_html(ln, al),
                       width=34).pack(pady=4)
        ...
        ttk.Button(btn_frame, text="📊 전체 지역 HTML 한번에 생성",
                   command=gen_all, width=34).pack(pady=4)
        ...
        ttk.Button(dialog, text="닫기", command=on_close, width=10).pack(pady=5)
    ...
    def show_notification_html_by_region(self):
        ...
        if not combined:
            messagebox.showinfo("알림", "재구성할 신고가 데이터가 없습니다.")
            return
...
class AptSelectDialog:
    def __init__(self, parent, apt_list, service_key, sigungu_code, dong, sido, sigungu, title="아파트 선택"):
        ...
        self.font_normal = (get_mac_font(), 9)
        self.font_large = (get_mac_font(), 11)
        self.font_button = (get_mac_font(), 9)
