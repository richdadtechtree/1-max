...
class RealEstateMonitorApp:
    ...
    def load_notifications_history(self):
        """DB에서 신고가 히스토리 로드"""
        try:
            # JSON 파일이 있으면 먼저 마이그레이션
            if os.path.exists(self.notifications_history_file):
                self.migrate_notifications_to_db()
            ...
            # 최근 30일 데이터만 로드
            cursor.execute("""
                SELECT id, apt_id, apt_name, price, prev_max_price, trade_date,
                       area, floor, timestamp
            ...
            for row in cursor.fetchall():
                noti_id, apt_id, apt_name, price, prev_max_price, trade_date, area, floor, timestamp = row

                # apt_list 재구성 (기존 형식과 호환성 유지)
                apt_list = [{
                    'apt_name': apt_name,
                    'new_max_price': price,
            ...
            logging.info(f"DB에서 신고가 히스토리 로드 완료 ({len(history_data)}개)")
            return history_data

        except Exception as e:
            logging.error(f"DB에서 신고가 히스토리 로드 중 오류: {str(e)}")
            import traceback
    ...
    def save_notifications_history(self):
        ...
        try:
            ...
            # 기존 30일 이전 데이터 삭제
            cutoff_date = datetime.now() - timedelta(days=30)
            cursor.execute("DELETE FROM notifications_history WHERE timestamp < ?",
                          (cutoff_date.strftime('%Y-%m-%d %H:%M:%S'),))

            # 새 데이터는 show_new_max_notification에서 직접 INSERT하므로
            # 여기서는 commit만 수행
            self.db_conn.commit()
            logging.info(f"신고가 히스토리 저장 완료")
    ...
    def restore_db_file(self, *, parent_window=None):
        ...
        try:
            ...
            # 신고가 히스토리 다시 로드
            self.load_notifications_history()
    ...
    def build_notification_html(self, apt_list):
        ...
        (function(){{
          ...
          // 초기 지역 목록
          const allRegions = Array.from(new Set(cards.map(c => c.dataset.region || '').filter(Boolean))).sort();
          ...
          function refreshRegionChips(){{
            ...
            // 칩을 전부 제거하고 다시 생성 (순서대로)
            chipsWrap.innerHTML = '';

            // '전체 보기' 칩 먼저 추가
            const allChip = document.createElement('span');
            allChip.className = 'region-chip' + (selectedRegion === '__ALL__' ? ' active' : '');
            allChip.dataset.region = '__ALL__';
            allChip.textContent = '전체 보기';
            chipsWrap.appendChild(allChip);
            ...
            sortedRegions.forEach(r=>{{
              ...
              const chip = document.createElement('span');
              chip.className = 'region-chip' + (selectedRegion === r ? ' active' : '') + (n === 0 ? ' zero' : '');
              chip.dataset.region = r;
          ...
          function setHeaderCount(n){{
            ...
            const filterOn = (
              ...
              selectedRegion !== '__ALL__' || rateMin.value || rateMax.value
            );
          ...
          function nonRegionMatch(card){{
            ...
            return passDate && passYear && passRate;
          }}
          ...
          chipsWrap.addEventListener('click', (e)=>{{
            ...
            $$('.region-chip', chipsWrap).forEach(c=>{{
              c.classList.toggle('active', c.dataset.region === selectedRegion);
            }});
            applyFilter();
          }});
    ...
    def show_new_max_notification(self, apt_list):
        ...
        try:
            ...
            if recent_count > 0 and recent_count == len(apt_list):
                is_duplicate = True
