...
class RealEstateMonitorApp:
    ...
    def analyze_kb_excel(self):
        ...
        try:
            ...
            # 엑셀 파일이 열려있을 수 있으므로 Win32 API로 공유 읽기 후 임시 파일로 복사
            temp_dir = tempfile.mkdtemp()
            temp_file = os.path.join(temp_dir, 'kb_temp.xlsx')
            try:
                # Win32 API: FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE = 7
                ...
                if h == INVALID_HANDLE_VALUE:
                    ...
                    logging.error(f"KB 엑셀 파일 열기 실패 (Win32 error {err}): {self.kb_excel_path}")
                    try:
                        os.rmdir(temp_dir)
                    except:
                        pass
                ...
                try:
                    # 파일 내용을 읽어서 임시 파일에 쓰기
                    buf_size = 1024 * 1024  # 1MB 버퍼
                    with open(temp_file, 'wb') as out_f:
                        while True:
                            buf = ctypes.create_string_buffer(buf_size)
                            bytes_read = ctypes.c_ulong(0)
                            success = kernel32.ReadFile(
                                h, buf, buf_size, ctypes.byref(bytes_read), None
                            )
    ...
    def show_settings_dialog(self):
        ...
        # KB 시세 엑셀 파일 경로
        ttk.Label(settings, text="KB 시세 엑셀 파일:", background=self.palette['surface'], foreground=self.palette['text_primary']).grid(row=5, column=0, sticky="w", padx=10, pady=10)
        kb_excel_var = tk.StringVar(value=self.kb_excel_path)
        ttk.Entry(settings, textvariable=kb_excel_var, width=40).grid(row=5, column=1, padx=5, pady=10)
        def select_kb_excel():
        ...
        ttk.Button(settings, text="찾아보기", style='Accent.TButton', command=select_kb_excel).grid(row=5, column=2, padx=5, pady=10)

        button_frame = ttk.Frame(settings, style='TFrame')
        button_frame.grid(row=6, column=0, columnspan=3, sticky="e", padx=10, pady=20)
        def save_settings():
            # 다운로드 경로
    ...
    def export_notification_html(self, apt_list, *, ask_path=False, silent=False, fixed_filename=None):
        ...
        try:
            ...
            if ask_path:
                ...
                # 자동 저장: newtrade 폴더에 고정 파일명
                script_dir = os.path.dirname(os.path.abspath(__file__))
                newtrade_dir = os.path.join(os.path.dirname(script_dir), "newtrade")
  ...
  <div class="tier-modal">
    ...
    def export_fear_greed_html(self):
        ...
        try:
            ...
            # 스크립트 위치의 상위 디렉토리에서 newtrade 폴더 찾기
            script_dir = os.path.dirname(os.path.abspath(__file__))
            newtrade_dir = os.path.join(os.path.dirname(script_dir), "newtrade")
            os.makedirs(newtrade_dir, exist_ok=True)
