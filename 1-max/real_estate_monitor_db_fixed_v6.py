...
총 추가된 단지: {total_added}개"""
    ...
    def build_notification_html(self, apt_list):
        ...
        for apt in apt_list:
            ...
            # 카카오맵 검색용 쿼리 (동 + 아파트명) - 안전하게 처리
            kakao_query = f"{location_dong} {name}".strip() if location_dong else name
    ...
    def build_area_ranking_html(self, area_type):
        ...
        for apt in filtered_apts[:MAX_RANK_DISPLAY]:
            ...
            if rank_change is not None:
                if rank_change > 0:  # 상승
                    rank_badge = f'<span class="rank-badge up">[▲{rank_change}]</span>'
                elif rank_change < 0:  # 하락
                    rank_badge = f'<span class="rank-badge down">[▼{abs(rank_change)}]</span>'
                else:  # 변동 없음
                    rank_badge = '<span class="rank-badge same">[-]</span>'
            else:  # 신규
                rank_badge = '<span class="rank-badge new">[★NEW]</span>'

            price_str = f"{apt['new_price']:,}만원" if apt['new_price'] else "-"
            date_str = apt['date'] or '-'
            floor_str = f"{apt['floor']}층" if apt['floor'] else '-'
            dong_str = apt['dong'] if apt['dong'] and apt['dong'] != '-' else ''
            # location_dong이 dict인 경우 빈 문자열로 처리
            loc_dong = apt['location_dong']
            if isinstance(loc_dong, dict):
                loc_dong = ''
            location = f"{apt['sido']} {apt['sigungu']} {loc_dong}"

            # 투기과열지구/토지거래허가구역 판정
            sido = apt['sido']
            sigungu = apt['sigungu']
            location_dong_raw = apt['location_dong']
            # location_dong이 dict인 경우 빈 문자열로 처리
            if isinstance(location_dong_raw, dict):
                location_dong = ''
            else:
            ...
            # 디버그: 성남시 아파트의 location_dong 값 확인
            if '성남' in sigungu:
                logging.info(f"[HTML 생성 디버그] {apt['apt_name']}: sido={sido}, sigungu={sigungu}, location_dong={location_dong}")
            ...
            # 경기 성남 (분당·수정·중원구)
                ...
                # 구 정보가 sigungu에 명시된 경우
                if '분당구' in sigungu or '수정구' in sigungu or '중원구' in sigungu:
                    is_hot = True
                    is_permit = True
                    logging.debug(f"[투기과열지구] {apt['apt_name']} - 구 정보로 판정: {sigungu}")
                # 동 이름으로 구 판별 (API에서 구 정보가 없는 경우)
                else:
                    # 분당구 동
                    if location_dong in ['구미동', '금곡동', '대장동', '백현동', '분당동', '서현동', '수내동', '야탑동', '운중동', '정자동', '판교동', '삼평동', '동막동', '궁내동', '율동', '매송동']:
                        is_hot = True
                        is_permit = True
                        logging.debug(f"[투기과열지구] {apt['apt_name']} - 분당구 동으로 판정: {location_dong}")
                    # 수정구 동
                    elif location_dong in ['고등동', '금토동', '단대동', '복정동', '신흥동', '양지동', '오야동', '태평동', '신촌동', '수진동', '창곡동', '시흥동', '둔전동']:
                        is_hot = True
                        is_permit = True
                        logging.debug(f"[투기과열지구] {apt['apt_name']} - 수정구 동으로 판정: {location_dong}")
                    # 중원구 동
                    elif location_dong in ['갈현동', '도촌동', '상대원동', '성남동', '은행동', '중앙동', '하대원동', '금광동', '여수동']:
                        is_hot = True
                        is_permit = True
                        logging.debug(f"[투기과열지구] {apt['apt_name']} - 중원구 동으로 판정: {location_dong}")
                    else:
                        logging.debug(f"[투기과열지구] {apt['apt_name']} - 성남시이지만 해당 없음: sigungu={sigungu}, location_dong={location_dong}")
            # 경기 수원 (영통·장안·팔달구)
            elif '수원' in sigungu:
                if '영통구' in sigungu or '장안구' in sigungu or '팔달구' in sigungu:
                    is_hot = True
                    is_permit = True
            # 경기 안양 (동안구)
            elif '안양' in sigungu:
            ...
            regulation_badges = ""
            if is_hot:
                regulation_badges += '<span class="badge-speculation hot">🔥 투기과열지구</span> '
            if is_permit:
                regulation_badges += '<span class="badge-speculation permit">🚧 토지거래허가구역</span>'

            # 가격대 분류 (data 속성용)
            price_range = ""
            price_val = apt['new_price']
            if price_val <= 100000:
                price_range = "under10"
            ...
            rank_info = ""
            if rank_change is not None:
                prev_rank = apt.get('prev_rank', 0)
                if rank_change > 0:
                    rank_info = f"📊 순위: {prev_rank}위 → {rank}위 ({rank_change}단계 상승)"
                elif rank_change < 0:
                    rank_info = f"📊 순위: {prev_rank}위 → {rank}위 ({abs(rank_change)}단계 하락)"
                else:
                    rank_info = f"📊 순위: 변동 없음"
            else:
                rank_info = "🆕 신규 진입"

            # 가격 변동 정보
            price_info = ""
            price_change = apt.get('price_change')
            prev_price = apt.get('prev_price')
            if price_change is not None and prev_price:
                if price_change > 0:
                    pct = (price_change / prev_price * 100) if prev_price > 0 else 0
                    price_info = f"📈 가격: {prev_price:,}만원 → {apt['new_price']:,}만원 (+{price_change:,}만원, +{pct:.1f}%)"
                elif price_change < 0:
                    pct = (abs(price_change) / prev_price * 100) if prev_price > 0 else 0
                    price_info = f"📉 가격: {prev_price:,}만원 → {apt['new_price']:,}만원 ({price_change:,}만원, -{pct:.1f}%)"
                else:
                    price_info = "📈 가격: 변동 없음"
            else:
                price_info = f"📈 가격: {apt['new_price']:,}만원 (신규)"

            # 연식 정보 처리
            build_year_str = ""
            if apt.get('build_year'):
                build_year = str(apt['build_year']).strip()
                if build_year and build_year != '':
                    if build_year == '분양':
                        ...
                        current_year = datetime.now().year
                        try:
                            year_num = int(build_year)
                            age = current_year - year_num
                            if age <= 5:
                                build_year_str = f'<span class="year-badge very-young">{build_year}년 ({age}년차)</span>'
                            elif age <= 10:
                                build_year_str = f'<span class="year-badge young">{build_year}년 ({age}년차)</span>'
                            else:
                                build_year_str = f'<span class="year-badge">{build_year}년 ({age}년차)</span>'
                        except:
                            build_year_str = f'<span class="year-badge">{build_year}년</span>'

            # 카카오맵 검색용 쿼리 (동 + 아파트명)
            kakao_query = f"{location_dong} {apt['apt_name']}".strip() if location_dong else apt['apt_name']
        ...
        (function(){{
          ...
          priceBtns.forEach(btn => {{
            btn.addEventListener('click', () => {{
              applyPriceFilter(btn.dataset.range);
            }});
          }});
    ...
    def build_price_distribution_html(self):
        ...
        # 3개월치 데이터를 JSON으로 변환
        all_trades_json = json.dumps(all_trades, ensure_ascii=False)
        ...
        <script>
          ...
          console.log('[가격대별 분위] 데이터 로드 완료');
          console.log('[가격대별 분위] 전체 데이터 건수:', allTrades.length);
          ...
          if (!allTrades || allTrades.length === 0) {{
            ...
            console.log('[가격대별 분위] 샘플 데이터:', allTrades.slice(0, 3));

            // 데이터에서 최대 가격 찾기
            maxPrice = Math.max(...allTrades.map(t => t.price));
          ...
          function aggregateByPriceBracket(trades, startDate, endDate) {{
            ...
            trades.forEach(trade => {{
              ...
              if (tradeDate >= start && tradeDate <= end) {{
                // 1억 단위로 가격대 결정
                const bracket = Math.floor(trade.price / 10000) * 10000;
          ...
          function handleChartClick(event, activeElements) {{
            ...
            const filteredTrades = allTrades.filter(trade => {{
              ...
              const start = new Date(startDate);
              const end = new Date(endDate);
              const tradeBracket = Math.floor(trade.price / 10000) * 10000;
